...
(async () => {
  ...
  try {
    ...
    if (routeTab) {
      ...
      // Hover over the center of the Leaflet map canvas where the polyline path is drawn
      console.log('Simulating mouse hover over route polyline on Leaflet map...');
      const mapDiv = await page.$('.leaflet-container');
      if (mapDiv) {
        const box = await mapDiv.boundingBox();
        if (box) {
          // Hover near middle of map
          await page.mouse.move(box.x + box.width * 0.52, box.y + box.height * 0.55);
          await page.waitForTimeout(1500);
        }
      }

      const routeMapPath = path.join(artifactDir, 'ui_route_map_hover_tooltip.png');
      await page.screenshot({ path: routeMapPath, fullPage: true });
      console.log(`Saved Hover Tooltip screenshot to ${routeMapPath}`);
    }

  } catch (err) {
    console.error('Error during Playwright Hover Tooltip test:', err);
  } finally {
    await browser.close();
  }
})();
