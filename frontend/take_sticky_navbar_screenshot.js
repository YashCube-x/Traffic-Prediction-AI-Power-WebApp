...
(async () => {
  ...
  try {
    ...
    if (routeTab) {
      ...
      console.log('Scrolling page down by 500px to test sticky navbar...');
      await page.evaluate(() => window.scrollTo(0, 500));
      await page.waitForTimeout(1000);

      const routeMapPath = path.join(artifactDir, 'ui_sticky_navbar.png');
      await page.screenshot({ path: routeMapPath });
      console.log(`Saved Sticky Navbar screenshot to ${routeMapPath}`);
    }

  } catch (err) {
    console.error('Error during Playwright Sticky Navbar test:', err);
  } finally {
    await browser.close();
  }
})();
